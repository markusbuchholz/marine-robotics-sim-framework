...
class BlueboatROSInterface(Node):
    def __init__(self):
        ...
        # Declare parameters
        self.declare_parameter('mavlink_connection', 'udpin:0.0.0.0:14560')
    ...
    def diff_drive_callback(self, msg):
        """
        Convert a Vector3 message from /blueboat/diff_drive into a Joy message.
        Mapping:
        - Vector3.x is used as the differential (steering) input → Joy.axes[0]
        - Vector3.y is used as the forward/backward input → Joy.axes[4]
        Only publish the simulated Joy message if the 'simulate_joy' parameter is set to True.
        """
        # Check whether simulation of the joystick is enabled.
        simulate_joy = self.get_parameter_or('simulate_joy', False)
        if not simulate_joy:
            self.get_logger().info("simulate_joy is False; ignoring simulated diff_drive input.")
            return

        from sensor_msgs.msg import Joy  # Ensure Joy is imported
        # Create a new Joy message with at least 5 axes.
        joy_msg = Joy()
        axes = [0.0] * 5
        # Mapping: use msg.x for differential and msg.y for forward/backward.
        axes[0] = msg.x   # differential input → Joy.axes[0]
        axes[4] = msg.y   # forward/backward input → Joy.axes[4]
        joy_msg.axes = axes
        joy_msg.buttons = []  # No buttons set
        self.joy_pub.publish(joy_msg)
        self.get_logger().info(f"Published simulated Joy message on /joy: axes={joy_msg.axes}")

    # Helper method to get parameter with default (since our rclpy version may not have get_parameter_or)
    ...
    def ___diff_drive_callback(self, msg):
        """
        Process Vector3 messages to control differential drive with two modes:
        1. Differential trimming: When forward (msg.y) is active, both motors move
            forward/backward with one motor’s speed reduced based on the differential (msg.x).
        2. Pivot-in-place: When forward is near zero, the differential input commands
            opposite speeds so the robot spins in place.
        """
        # Extract joystick inputs from the Vector3:
        #   - msg.y is the forward/backward command (expected normalized to [-1, 1])
        #   - msg.x is the differential command (expected normalized to [-1, 1])
        forward_input = float(msg.x)
        diff_input = float(msg.y)
        self.get_logger().info(f"Diff_drive command received: forward_input={forward_input}, diff_input={diff_input}")

        # Define control parameters.
        max_thrust = 100.0    # Full power value.
        trim_scale = 5.0      # Scaling factor for differential trimming.
        deadband = 0.1        # Threshold for forward command activation.

        # Mode 1: If forward command is active, perform differential trimming.
        if abs(forward_input) > deadband:
            baseline = forward_input * max_thrust
            if diff_input > 0:
                # Positive differential: reduce port (left) motor.
                port_thrust = baseline * (1 - trim_scale * abs(diff_input))
                stbd_thrust = baseline
            elif diff_input < 0:
                # Negative differential: reduce starboard (right) motor.
                port_thrust = baseline
                stbd_thrust = baseline * (1 - trim_scale * abs(diff_input))
            else:
                port_thrust = baseline
                stbd_thrust = baseline
        else:
            # Mode 2: Pivot-in-place.
            port_thrust = diff_input * max_thrust
            stbd_thrust = -diff_input * max_thrust

        self.get_logger().info(f"Computed motor thrusts: port={port_thrust}, stbd={stbd_thrust}")
        # Instead of publishing, send commands via send_motor_thrust.
        self.send_motor_thrust(3, int(port_thrust))
        self.send_motor_thrust(4, int(stbd_thrust))
...
class JoystickNode(Node):
    ...
    def direct_joy_callback(self, msg):
    ...
    def joy_callback(self, msg):
        """
        Process joystick messages to control differential drive with two modes:
        1. Differential trimming: When forward axis is active, both motors move forward
            with one motor’s speed reduced based on the differential axis.
        2. Pivot-in-place: When forward axis is neutral, the differential axis commands
            opposite speeds for the motors so the robot spins in place.
        """
        # Log the received joystick data.
        self.get_logger().info(f"Joy message received: buttons: {msg.buttons}, axes: {msg.axes}")

        # Process buttons for arm/disarm commands.
        if len(msg.buttons) >= 8:
            if msg.buttons[7] == 1:
                self.get_logger().info("Button 7 pressed: Arming vehicle")
                self.blueboat.arm_vehicle()
        ...
        if len(msg.axes) > 4:
            ...
            forward_input = float(msg.axes[4])      # forward/backward axis, expected [-1, 1]
            diff_input = float(msg.axes[0])           # differential axis, expected [-1, 1]

            # Define parameters.
            max_thrust = 100.0         # full power value for each motor.
            trim_scale = 5.0           # scaling factor for differential trimming.
            deadband = 0.1             # threshold for forward axis activation.

            # Two modes: forward (or backward) drive versus pivot-in-place.
            if abs(forward_input) > deadband:
                # Mode 1: Differential trimming while moving forward/backward.
                baseline = forward_input * max_thrust
                if diff_input > 0:
                    # Positive differential: reduce port (left) motor.
                    port_thrust = baseline * (1 - trim_scale * abs(diff_input))
                ...
                # Mode 2: Pivot-in-place.
                # When forward is neutral, the differential axis drives the motors in opposite directions.
                # For example, if diff_input is positive, port motor spins forward and starboard spins reverse.
                port_thrust = diff_input * max_thrust
                stbd_thrust = -diff_input * max_thrust
            ...
            self.port_motor_pub.publish(port_msg)
            self.get_logger().info(f"Published port motor thrust: {port_thrust}")

            stbd_msg = Float32()
            stbd_msg.data = stbd_thrust
            self.stbd_motor_pub.publish(stbd_msg)
            self.get_logger().info(f"Published starboard motor thrust: {stbd_thrust}")
        else:
            self.get_logger().warn("Insufficient axes data for differential control. At least 5 axes are required.")
