...
ros2 topic pub -1 /waypoints std_msgs/Float64MultiArray "{data: [0.0, 5.0]}"
"""
...
class SetYawController(Node):
    def __init__(self):
        ...
        # NEW: Subscribe to /waypoints (Cartesian waypoint as [x, y])
    ...
    def waypoint_callback(self, msg: Float64MultiArray):
        """
        Callback for the /waypoints topic.
        The incoming message is expected to contain at least two values [x, y].
        
        The boat is assumed to be at (0,0) in the Cartesian coordinate system.
        In many navigation systems, 0° is North and 90° is East, whereas math.atan2
        returns 0° along the positive x-axis (East). Therefore, we convert the math angle
        to a navigation heading using:
        
            desired_heading = (90 - math.degrees(atan2(y, x))) mod 360
        
        Examples:
          - For [5.0, 5.0]: math_angle=45° → desired_heading = 45°
          - For [5.0, 0.0]: math_angle=0°  → desired_heading = 90°
          - For [5.0, -5.0]: math_angle=-45° → desired_heading = 135°
        """
        if not msg.data or len(msg.data) < 2:
            self.get_logger().error("Waypoints message must contain at least two values [x, y].")
            return

        x, y = msg.data[0], msg.data[1]
        # Compute the math angle (in degrees) measured from the positive x-axis (East)
        math_angle = math.degrees(math.atan2(y, x))
        # Convert to navigation heading (0° is North, 90° is East)
        desired_heading = (90 - math_angle) % 360
        self.get_logger().info(f"Waypoint received: ({x:.1f}, {y:.1f}). Computed desired heading: {desired_heading:.1f}°.")

        # Automatically update the desired yaw by synthesizing a set_yaw message.
        set_yaw_msg = Float32()
        set_yaw_msg.data = desired_heading
        self.set_yaw_callback(set_yaw_msg)
